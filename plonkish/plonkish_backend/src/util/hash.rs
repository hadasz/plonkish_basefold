...
use sha3::digest::{Digest, HashMarker,OutputSizeUser,Reset,FixedOutput};
use generic_array::GenericArray;
use std::fmt::Debug;
use blake2b_simd::{State, many::{HashManyJob,hash_many},Hash as BHash,OUTBYTES,Params};
...
impl Hash for Blake2s{
    fn update_consec_field_pairs(values: &Vec<impl PrimeField>) -> Vec<Output<Self>>{
	let v = values.par_chunks_exact(2).map(|c| [c[0].to_repr().as_ref(),c[1].to_repr().as_ref()].concat()).collect::<Vec<Vec<u8>>>();
	let mut params = Params::new();
	params.hash_length(Self::OUTPUTLEN);
	let mut jobs = v.iter().map(|v| HashManyJob::new(&params,v)).collect::<Vec<HashManyJob>>();
	...
	let v = values.par_chunks_exact(2).map(|c| [c[0],c[1]].concat()).collect::<Vec<Vec<u8>>>();
	...
	let outputs:Vec<Output<Self>> = jobs.iter().map(|job| {
	    *GenericArray::from_slice(job.to_hash().as_bytes())
	}).collect();
